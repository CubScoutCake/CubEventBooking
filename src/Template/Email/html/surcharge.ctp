 ...
 */
?>
<h2>Balance Outstanding - Late Payment Fee Added</h2>

<p>Your Invoice (number <?= $this->Number->format($invoice_id) ?>) currently has a balance <strong>outstanding</strong> of <strong><?= $this->Number->currency($balance,'GBP') ?></strong>. This has increased by <?= $this->Number->format($percentage) ?>&percnt; as a late payment surcharge of <?= $this->Number->currency($fee,'GBP') ?> was added. </p>

<p><strong>Please arrange payment as soon as possible as this surcharge will increase if the balance is not recieved before the date of Event.</strong></p>
