...
<p>We have recorded a payment of <?= $this->Number->currency($payment_value,'GBP') ?> against your invoice (number <?= $this->Number->format($invoice_id) ?>).</p>

<p>This invoice now has a balance of <strong><?= $this->Number->currency($balance,'GBP') ?></strong>.</p>
<p>The initial value of this invoice was <?= $this->Number->currency($initialvalue,'GBP') ?> and the total sum of payments received on this invoice to date is <?= $this->Number->currency($value,'GBP') ?>.<p>
...
, 'invoice_id' => $invoice->display_code -->
