...
class Sections extends AbstractMigration
{
    ...
    {
        $table = $this->table('users');
        $table->dropForeignKey('users_scoutgroup_id')
            ->removeIndex(['scoutgroup_id'])
            ->removeColumn('scoutgroup_id')
            ->update();

        $table = $this->table('applications');
        $table->dropForeignKey('applications_scoutgroup_id')
            ->removeIndex(['scoutgroup_id'])
            ->removeColumn('scoutgroup_id')
            ->update();

        $table = $this->table('attendees');
        $table->dropForeignKey('attendees_scoutgroup_id')
            ->removeIndex(['scoutgroup_id'])
            ->removeColumn('scoutgroup_id')
            ->update();
    }
    ...
    {
        $table = $this->table('users');
        $table->addColumn('scoutgroup_id', 'integer', [
                'default' => null,
                'limit' => 11,
                'null' => true,
            ])
